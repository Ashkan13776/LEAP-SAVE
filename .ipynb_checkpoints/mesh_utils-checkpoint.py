...
def create_graph_with_features(num_points, k, surface_bias=2):
    ...
    for i, pos in enumerate(positions):
        ...
        # Adding position and distance from center as node attributes
        G.add_node(i, pos=pos, distance_from_center=distance_from_center)

    tree = KDTree(positions)
    for i in range(num_points):
        distances, indices = tree.query(positions[i], k+1)  # +1 to exclude the point itself
        for index in indices[1:]:  # Exclude the point itself
            G.add_edge(i, index)
